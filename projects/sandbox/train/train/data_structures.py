...
def sample_kernels(chunk, kernel_size, batch_size):
    """
    Randomly sample `batch_size` kernels from `chunk`, which
    is actually a  list of chunks, each corresponding to a
    different background file.
    """
...
class ChunkedDataloader:
    ...
    Args:
        fnames:
            A list of file names from which to read data.
            Each should be an HDF5 file with datasets
            corresponding to the values in `ifos`.
        ifos:
            A list of interferometer names. For each
            interferometer, there should be a dataset in each
            file of `fnames`.
        kernel_length:
            The length, in seconds, of each batch element that
            the network will see
        sample_rate:
            The frequency, in Hz, at which the data in `fnames`
            has been sampled
        batch_size:
            Number of samples over which the network will compute
            each gradient update during training.
        reads_per_chunk:
            Number of files from which to load a `chunk_length`
            amount of data from at one time
        chunk_length:
            The length, in seconds, of data to load from each file
            at one time
        batches_per_chunk:
            Number of batches to sample from each of the `chunk_length`
            chunks across the `reads_per_chunk` files
        chunks_per_epoch:
            Number of chunk loads to do per epoch, resulting in
            `chunks_per_epoch * batches_per_chunk` batches per epoch
        device:
            Device on which to perform training. Either `"cpu"`,
            `"cuda"`, or `"cuda:<device index>"` to train on a
            specific GPU.
        preprocessor:
            Preprocessing module to pass the chunked data through
            prior to handing it to the main network
    """
...
class GlitchSampler(torch.nn.Module):
    """
    This will be made obsolete by the ChunkedGlitchLoader
    """
