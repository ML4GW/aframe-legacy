...
def generate_glitch_dataset(
    ...
    Args:
        snr_thresh: SNR threshold above which glitches will be kept
        start: GPS time at which to begin looking for glitches
        stop: GPS time at which to stop looking for glitches
        window: Amount of time in seconds on either side of a glitch to
            query data for
        sample_rate: Sample rate of queried data
        channel: Channel name used to read data. Should include the
            interferometer prefix
        trigger_files: List of h5 files containing omicron triggers
        chunk_size: Length in seconds of data to query at one time
...
def main(
    ...
    Args:
        snr_thresh: SNR threshold above which glitches will be kept
        start: GPS time at which to begin looking for glitches
        stop: GPS time at which to stop looking for glitches for the
            training dataset. Marks the beginning of the testing dataset
        test_stop: GPS time at which to stop looking for glitches for the
            testing dataset
        ...
        chunk_duration: Duration of data in seconds for PSD estimation
        segment_duration: Duration of data in seconds for FFT
        overlap: Overlap in seconds between neighbouring segments and chunks
        mismatch_max: Maximum distance between (Q, f) tiles
        window: Amount of time in seconds on either side of a glitch to
            query data for
        datadir: Directory to which the glitch dataset will be written
        logdir: Directory to which the log file will be written
        channel: Channel name used to read data. Should not include the
            interferometer prefix
        frame_type: Frame type for data discovery with gwdatafind
        sample_rate: Sample rate of queried data
