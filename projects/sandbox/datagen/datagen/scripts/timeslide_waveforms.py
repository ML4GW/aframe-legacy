...
def main(
    ...
    Args:
        start: GPS time of the beginning of the testing dataset
        stop: GPS time of the end of the testing dataset
        shifts: A list of shifts in seconds. Each value corresponds to the
        ...
        background: Directory containing background data to use for PSD
            calculation. Should have data for each interferometer
        spacing: The spacing between signals in seconds
        buffer: The buffer to apply to each side of the segment in seconds
        cosmology: A function that returns an astropy cosmology when called
        prior: A function that returns a Bilby PriorDict when called
        minimum_frequency: Minimum_frequency for generating waveform; not to
            be confused with highpass filter frequency
        reference_frequency: Reference frequency for generating waveform
        sample_rate: Rate at which to sample time series
        waveform_duration: Duration of waveform in seconds
        waveform_approximant: Name of the waveform approximant to use.
        highpass: The frequency to use for a highpass filter
        snr_threshold: Minimum SNR of generated waveforms. Sampled parameters
...
def deploy(
    ...
    Args:
        ...
        Tb: The length of background time to be generated via time shifts
        shifts: A list of shifts in seconds. Each value corresponds to the
            the length of time by which an interferometer's timeseries is
            moved during one shift. For example, if `ifos = ["H1", "L1"]`
            and `shifts = [0, 1]`, then the Livingston timeseries will be
        ...
        spacing: The spacing between signals in seconds
        buffer: The buffer to apply to each side of the segment in seconds
        min_segment_length: The shortest a contiguous segment of background
        ...
        accounting_group_user: Username of the person running the condor jobs
        accounting_group: Accounting group for the condor jobs
        request_memory: Amount of memory for condor jobs to request in Mb
